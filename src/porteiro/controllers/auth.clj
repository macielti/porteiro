...
(s/defn customer-authentication! :- s/Str
  [{:customer-auth/keys [username password]} :- models.auth/CustomerAuth
   ...
   producer
   database-connection]
  (let [{:customer/keys [hashed-password id] :as customer} (database.customer/by-username username database-connection)
        {:contact/keys [email]} (first (database.contact/by-customer-id id database-connection))]
