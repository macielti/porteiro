...
(deftest update-password-test
  (let [system     (components/start-system!)
        service-fn (-> system :server :server :io.pedestal.http/service-fn)
        _          (http/create-user user service-fn)
        {{:keys [token]} :body} (http/auth user-auth service-fn)]
    ...
    (testing "that we can update password"
      (is (match? {:status 204
                  ...
                  (http/update-password password-update token service-fn))))
    ...
    (testing "that i can't update a password if the old one is incorrect"
      (is (match? {:status 403,
                  ...
                  (http/update-password (assoc password-update :oldPassword "wrong-old-password") token service-fn))))

    (testing "should return a nice and readable response in case of wrong input"
      (is (match? {:status 422,
                   :body   {:cause {:oldPassword "missing-required-key"}}}
                  (http/update-password (dissoc password-update :oldPassword) token service-fn))))
    ...
    (testing "shouldn't be able to change update a password with a inavalid jwt token"
      (is (match? {:status 422
                  ...
                  (http/update-password password-update "invalid-jwt-token" service-fn))))
